...
def create_salesorder_item(db: Session, salesorder_id: int, item: SalesorderItemCreate) -> SalesorderItemResponse:
    ...
    try:
        with db.begin():  # Start transaction
            ...
            # Prepare data and exclude 'salesoderitems_id'
            item_data = item.model_dump(exclude={"salesoderitems_id"})

            # Create and add the new SalesorderItems
...
def create_salesorder(db: Session, salesorder_data: SalesOrderCreate) -> SalesOrder:
    """
    ...
    try:
        ...
        return SalesOrder.from_orm(new_salesorder)
    except IntegrityError as e:
        db.rollback()
        raise HTTPException(status_code=400, detail=f"Integrity error: {e.orig}")
    except Exception as e:
...
def get_salesorder_by_id(db: Session, salesorder_id: int) -> SalesOrder:
    """
    ...
    if not salesorder:
        raise HTTPException(status_code=404, detail="Sales order not found")
    return SalesOrder.from_orm(salesorder)

def get_all_salesorders(db: Session, skip: int = 0, limit: int = 100) -> List[SalesOrder]:
    """
    Retrieve all sales orders with pagination.
    """
    salesorders = db.query(SalesOrder).offset(skip).limit(limit).all()
    return [SalesOrder.from_orm(salesorder) for salesorder in salesorders]

def update_salesorder(db: Session, salesorder_id: int, update_data: SalesOrderUpdate) -> SalesOrder:
    """
